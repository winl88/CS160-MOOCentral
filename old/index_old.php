	...
	<head>
		...
		<script src="http://code.jquery.com/jquery-1.11.0.min.js"></script>
 		...
 		<script>
			...
			function rate( id, which )
			{
                   		if (which == 'up'){
                   			$('#rate_count_'+id).html(parseInt($('#rate_count_'+id).html(), 10)+1);
                   		}else if (which == 'down'){
                   			$('#rate_count_'+id).html(parseInt($('#rate_count_'+id).html(), 10)-1);
                   		}
			}
...
<?php
	...
	$data = mysqli_query($con, "Select * from course_data join coursedetails where coursedetails.course_id = course_data.id and course_data.start_date >= CURDATE()");
